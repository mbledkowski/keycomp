...
const getElement = () => {
  ...
  const fetchSwitches = async () => {
    let { data, error } = await supabase.from('switches').select('brand,name,profile,loudness,travel,stem,socket,mount,pressure,actuation,bottom,preTravel,totalTravel,topC, stemC, bottomC')
    data = data || []
    if (error) {
  ...
  return {
    entry, photos, sources, models, modelGroups, switchName, fetchSwitch, fetchSwitchPhotos, fetchSwitchSources, fetchSwitchModels, page, fetchSwitches,
  }
}
